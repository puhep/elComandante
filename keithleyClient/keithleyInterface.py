...
#        self.setFO
    ...
    def doLinearSweep(self,startValue,stopValue,stepValue,nSweeps,delay):
        ...
        if self.maxVolt < math.fabs(startValue):
            startValue= math.copysign(self.maxVolt,startValue)
            print 'set start voltage to maximum allowed voltage: %s'%startValue
        self.setVoltageSweepStartValue(startValue)
        if self.maxVolt < math.fabs(stopValue):
            stopValue = math.copysign(self.maxVolt,stopValue)
            print 'set stop voltage to maximum allowed voltage: %s'%stopValue
    ...
    def initKeithley(self,protection = 500e-6):
        ...
        self.clearErrorQueue()
        #self.setComplianceAbortLevel('EARLY')
